...
def evaluate_TSD_contain_NULL(path, best_epoch_file, tag_schema):
	with io.open(os.path.join(path, TXT_file(best_epoch_file)), 'r', encoding='utf-8') as f_pre:
		Gold_Num = 0
		True_Num = 0
		Pre_Num = 0
		tag_schema == 'TO'
...
def evaluate_TASD(path, epochs, tag_schema):
	...
	for index in range(epochs):
		...
		with io.open(os.path.join(path, TXT_file(file_pre)), 'r', encoding='utf-8') as f_pre:
			Gold_Num = 0
			True_Num = 0
			Pre_Num = 0
			NULL_Gold_Num = 0
